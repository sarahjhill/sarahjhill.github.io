...
Exposed as window.SJHFire.create(canvas, opts) so more than one
scene can breathe fire independently — the hero dragon uses one
instance below, and the comic's dragon (see comic-dragon.js) creates
its own. Each instance gets its own mouth object; whichever 3D layer
owns that mouth writes the current screen position into it every
frame, and this file just spawns particles once it's marked active
and reads wherever it currently is — so the flame tracks the model
without either file needing to know the other's internals.
...
(function () {
	...
	/* the soft blob is drawn once into an offscreen canvas and stamped
	   with drawImage — building a radial gradient for every particle on
	   every frame is what made this expensive, and one sprite covers
	   every instance since none of them recolor it */
	var SPRITE = (function () {
	...
	function create(canvas, opts) {
		...
		/* dirX/dirY: unit vector the mouth is currently pointing. Whichever
		   3D layer owns this instance writes it every frame from the
		   model's actual facing. */
		var mouth = opts.mouth || { x: 0, y: 0, s: 1, active: false, dirX: -1, dirY: 0 };
		...
		var MAX_FLAMES = opts.maxFlames || 130;
		var flames = [];
		function spawn() {
		...
		resize();
		/* staticWhenReduced: the hero dragon is always there, so reduced
		   motion still gets one still frame of fire. The comic's dragon
		   only ever appears mid-animation (a torch sequence reduced-motion
		   visitors never trigger), so it opts out — there's nothing to
		   freeze a still frame of at load time. */
		if (reduce && opts.staticWhenReduced !== false) {
			mouth.active = true;
		...
		return { mouth: mouth, start: start, stop: stop, resize: resize };
	}

	window.SJHFire = { create: create };

	/* ---- hero instance, wired up exactly as before ---------------------- */
	var heroCanvas = document.querySelector('.sjh-dragon-canvas');
	if (heroCanvas) {
		window.__dragonMouth = window.__dragonMouth || { x: 0, y: 0, s: 1, active: false, dirX: -1, dirY: 0 };
		create(heroCanvas, { mouth: window.__dragonMouth });
	}
}());
